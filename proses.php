            ...
            if (move_uploaded_file($_FILES["fileMentah"]["tmp_name"], $targetLokasi)) {
                ...
                foreach (array_slice($data, 1) as $baris) {
                    ...
                    $hotspotData[] = [
                        ...
                        'kepercayaan' => @$dataPecah[$hitungBaris][5],
                        'provinsi' => trim(strval(@$dataPecah[$hitungBaris][10]), '"'),
                        'kabupaten' => trim(strval(@$dataPecah[$hitungBaris][9]), '"'),
                        'kecamatan' => trim(strval(@$dataPecah[$hitungBaris][8]), '"'),
                        'satelit' => strtoupper(strval(@$dataPecah[$hitungBaris][6])),
                        'tanggal' => @$dataPecah[$hitungBaris][1],
                        'waktu' => @$dataPecah[$hitungBaris][2],
                        'radiusKemungkinan' => @$dataPecah[$hitungBaris][7],
                        'tipe' => trim(trim(strval(@$dataPecah[$hitungBaris][11])), '"'),
                    ];
                }
